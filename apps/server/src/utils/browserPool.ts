...
class BrowserPool {
  ...
  private async getBrowser(): Promise<Browser> {
    ...
    logger.info("[BrowserPool] Initializing shared Chromium browser instance...")
    this.initializing = chromium.launch({
      headless: true,
      args: [
        "--no-sandbox",
      ...
      logger.info("[BrowserPool] Shared browser instance started successfully.")
      return b
    }).catch((err) => {
      this.initializing = null
      logger.error("[BrowserPool] Failed to launch Chromium browser: " + err.message)
      throw err
    })
