...
const generateSummary = (markdownText: string): string => {
  ...
  if (sentences.length > 0) {
    ...
    bullets.push("*   **Analysis**: Unable to locate text elements of high density.")
  }
  return `### AI Summary\n` + bullets.join('\n')
}
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  const optionsKey = Buffer.from(JSON.stringify(activeOptions)).toString("base64")
  ...
  try {
    ...
    try {
      ...
      if (activeOptions.summary) {
        ...
        json.summaryBullets = summary.split("\n").filter(s => s.startsWith("*")).map(s => s.replace(/^\*\s+/, ""))
      }
      ...
      if (activeOptions.branding) {
        const branding = await page.evaluate(() => {
          ...
          for (const el of elements.slice(0, 300)) {
            ...
            if (bg && bg !== '#FFFFFF' && bg !== '#000000' && bg !== '#TRANSPARENT') colors.add(bg)
          ...
          return {
            colors: [
              ...
              { name: "Text Primary", hex: textPrimary, desc: "Body Text Shade" },
              { name: "Link Color", hex: linkColor, desc: "Anchor Element Tint" }
            ],
            typography: {
              fontFamily: bodyStyle ? (bodyStyle.fontFamily.split(',')[0] ?? "sans-serif").replace(/['"]/g, '') : "sans-serif",
              heading: h1Style ? (h1Style.fontFamily.split(',')[0] ?? "sans-serif").replace(/['"]/g, '') : "sans-serif",
      ...
      try {
        ...
        if (user && user.email) {
          await pool.query(
            ...
            [user.email, url, JSON.stringify(activeOptions), JSON.stringify(result)]
          )
    ...
    return res.status(500).json(new ApiResponse(500, null, err.message || "Failed to execute scrape operation"))
  }
})
...
router.get("/runs", authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT id, url, options, created_at FROM scrape_runs WHERE user_email = $1 ORDER BY created_at DESC LIMIT 10",
      [user.email]
    )
    return res.status(200).json(new ApiResponse(200, result.rows, "Scrape runs retrieved successfully"))
  } catch (err: any) {
    logger.error(`[Scraper] Error fetching runs: ${err.message}`)
    return res.status(500).json(new ApiResponse(500, null, "Failed to retrieve recent runs"))
  }
})
...
router.get("/runs/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json(new ApiResponse(404, null, "Scrape run not found"))
    }

    return res.status(200).json(new ApiResponse(200, result.rows[0].result, "Scrape run retrieved successfully"))
  } catch (err: any) {
    logger.error(`[Scraper] Error fetching run details for ID ${id}: ${err.message}`)
    return res.status(500).json(new ApiResponse(500, null, "Failed to retrieve scrape run details"))
  }
})
